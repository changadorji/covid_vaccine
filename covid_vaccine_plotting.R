...
#plotting
df_plot <- df_filtered %>%
  count(month, month1, Country_Region, wt=dose)

df_plot %>%
  ggplot(aes(as.numeric(month), n, colour=Country_Region))+
  geom_textline(aes(label=Country_Region), hjust=.0001, size=6, linewidth=1.5)+
  geom_text(data = subset(df_plot, month=='12'),
                       aes(x=12, y=n, label=Country_Region), size=6, hjust=-.1)+
  ...
  annotation_custom(tableGrob(table, theme=tt), xmin = .5, xmax=5, ymin=180, ymax = 350)
